...
const creatorKeypairBase58 = process.env.CREATOR_KEYPAIR;
if (!creatorKeypairBase58) {
  throw new Error("CREATOR_KEYPAIR is not defined in the .env file");
}
const creator = Keypair.fromSecretKey(bs58.decode(creatorKeypairBase58));
const secondMember = Keypair.generate();

// Specify the recipient address here
const recipientAddress = new PublicKey(''); // Address to send the SOL
async function main() {
  const multisigModule = await import("@sqds/multisig");
  const multisig = multisigModule.default || multisigModule;
